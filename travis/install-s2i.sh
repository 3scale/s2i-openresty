...
if [ ! -f "${PREFIX}/s2i" ]; then
  cd /tmp/
  mkdir -p "${PREFIX}"
  wget -T 60 -q -c https://github.com/openshift/source-to-image/releases/download/v${version}/source-to-image-v${version}-${sha}-${os}-${arch}.tar.gz -O source-to-image.tar.gz
